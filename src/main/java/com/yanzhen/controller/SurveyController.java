...
public class SurveyController {
    ...
    public Map<String,Object> publish(Integer id,HttpServletRequest request){
        ...
        String url = "http://"+IPUtils.getServerIp()+ ":" + request.getServerPort() + request.getContextPath()+uuid;
        survey.setUrl(url);
        surveyService.update(survey);
        return MapControl.getInstance().success().getMap();
    }
    ...
    @GetMapping("/excel/{id}")
    public String excel(@PathVariable("id") Integer id,ModelMap modelMap){
        List<Map<String, Object>> list = surveyService.excel(id);
    ...
    //分享问卷
    @GetMapping("/share/{id}")
    public String share(@PathVariable("id") Integer id,ModelMap modelMap,HttpServletRequest request){
        Survey survey = surveyService.detail(id);
        if(!StringUtils.hasLength(survey.getUrl())){
            modelMap.put("msg","请先发布问卷");
            return "survey/share";
        }else{
            //上传的位置
            String path = request.getSession().getServletContext().getRealPath("/upload/");
            ...
            String text = survey.getUrl();  //这里设置自定义网站url
            String destPath = path;
            try {
                String absPath = QRCodeUtils.encode(text,null,destPath, true,survey.getId().toString());
                String pathz = request.getContextPath();
                String basePath = request.getScheme()+"://"+IPUtils.getServerIp()+":"+request.getServerPort()+pathz+"/";
                modelMap.put("path",basePath+"upload/"+survey.getId()+".jpg");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public String query_detail(@PathVariable("id") Integer id,ModelMap modelMap){
        ...
        for (Question question1 : questions) {
            if(question1.getType() == 3 || question1.getType() == 4){//处理问答题
                List<AnswerTxt> answerTxtList = new ArrayList<>();
                for (AnswerTxt txt : answerTxts) {
                    if(question1.getId() == txt.getQuestionId().intValue()){
                        answerTxtList.add(txt);
                    }
                }
                question1.setAnswerTxts(answerTxtList);
            }else{//处理选择题
