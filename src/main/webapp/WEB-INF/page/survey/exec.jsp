                ...
                <c:if test="${question.type == 1  or question.type == 2 }">
                    <div class="radioTemplate spliter">
                        <li class="using radio">
                            <input type="hidden" must="${question.required}"  name="questionId" xtype="${question.type}" value="${question.id}">
                            <div class="title">
                                <div class="no_edit">
                                    <c:if test="${question.required == 1}"><span style="color: red;">*</span></c:if>
                                    <strong>${status.index+1}、${question.title}</strong>
                                </div>
...
<script>
    layui.use(['jquery','layer'],function () {
        ...
        layer.prompt({title: '验证密码',closeBtn :0},function(value, index, elem){
            if(value == "${password}"){
                layer.close(index);
            }else{
        ...
        $('.mysave').click(function(){
            ...
            questionArr.each(function(index){
                ...
                if((obj.type == 1 || obj.type == 2)  && obj.must == "1" && obj.opts.length == 0){
                    flag = false;
                }
                if((obj.type == 3 || obj.type == 4)  && obj.must == "1" && obj.result == ""){
                    flag = false;
                }
                list.push(obj);
            })
